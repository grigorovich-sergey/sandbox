...
As a scientist, I try to change as few parts of a model as possible at once, so the results remain interpretable.
...
This question also matters beyond RNA. Practical quantum computers are likely to operate as parts of **hybrid quantum–classical systems**. If a step can be done reliably classically, there is little reason to spend quantum resources on it. So as quantum systems grow, the question of **what should stay outside the quantum circuit** will appear again and again.

---

## Slide 2 — One Controlled Experiment

I compare three QUBO formulations with different constrains placement. Everything else is held constant, so the main variable is where conflicts are resolved.

**Strict** penalizes overlap and crossing stems.

**Relaxed** penalizes overlap, but repairs crossings after sampling.

**Postprocessed** rewards stems only and repairs all conflicts classically.

---

## Slide 3 — What I Actually Tested

I completed **more than a thousand successful runs**: 600 Aer simulations and **500 IBM hardware runs**.

Hardware experiments reached **44-nucleotide sequences** and **143 logical variables** on IBM Quebec Sustem One.

For each run, I measured circuit cost, raw validity, repair burden, and ViennaRNA-based quality.

I also separated whether a good candidate was sampled from whether it could be selected without the reference.

---

## Slide 4 — Some Constraints Can Move Out

Relaxed encoding used **37 percent fewer quadratic interactions** than strict and reduced mean circuit depth by **21 percent**.
...
Postprocessed encoding kept the circuit cheap, but shifted too much work to repair.

It required more repair and produced weaker sampled candidates.

So the two extremes both have costs: strict is expensive, while postprocessed removes too much structure.
...
The result appears robust: relaxed quality still stays close to strict while keeping about **20 percent lower circuit depth**. Postprocessed improves, but remains weaker.
...
And that brings us to the key design question:

> **It is not how many constraints we can encode, but which constraints are worth paying quantum resources for.**
